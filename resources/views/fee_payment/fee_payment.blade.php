...
<div class="bg-gray-100 py-6">
    <div class="bg-white p-6 rounded-lg shadow-lg">
        <h2 class="text-2xl font-bold mb-4 text-teal-700">
            <i class="fa-solid fa-credit-card"></i> Fee & Payment Management
        </h2>
    ...
    <div id="paymentModal" class="fixed inset-0 bg-gray-900 bg-opacity-50 hidden flex items-center justify-center">
        <div class="bg-white p-6 rounded-lg shadow-lg w-96">
            <h3 id="modalTitle" class="text-xl font-bold mb-4 text-teal-700">
                <i class="fa-solid fa-edit"></i> Add Payment
            </h3>
            <input type="hidden" id="paymentId">
            <input type="text" id="studentName" placeholder="Student Name" class="w-full p-2 border rounded mb-2">
            <input type="text" id="feeType" placeholder="Fee Type (e.g. Tuition, Hostel)" class="w-full p-2 border rounded mb-2">
            <input type="number" step="0.01" id="amount" placeholder="Amount" class="w-full p-2 border rounded mb-2">
            <input type="date" id="paymentDate" class="w-full p-2 border rounded mb-2">
            <input type="text" id="paymentMethod" placeholder="Payment Method (e.g. Cash, Card)" class="w-full p-2 border rounded mb-2">
            ...
            <div class="flex justify-end">
                ...
                <button onclick="savePayment()" class="bg-teal-600 hover:bg-teal-700 text-white px-4 py-2 rounded">
    ...
    <script>
        ...
        function openModal(id = null) {
            if (id) {
                $.get(`/payments/${id}`, function(payment) {
                    ...
                    $('#paymentStatus').prop('checked', payment.status);
                });
            } else {
                ...
                response.data.data.forEach((payment,index) => {
                    table.append(`
                        <tr>
                            ...
                            <td class="border p-2 text-center">${payment.status ? 'Confirmed' : 'Pending'}</td>
                            <td class="border p-2 text-center">
                                <button onclick="openModal(${payment.id})" class="bg-yellow-500 text-white px-2 py-1 rounded mr-2">Edit</button>
                                <button onclick="deletePayment(${payment.id})" class="bg-red-500 text-white px-2 py-1 rounded">Delete</button>
                            </td>
                        </tr>
                    `);
                });
        ...
        function updatePagination(data) {
            ...
            pagination.html('');
            pagination.append(`
                <span>Showing ${data.from || 0} to ${data.to || 0} of ${data.total} payments</span>
                <div>
        ...
        function savePayment() {
            ...
            let paymentData = {
                ...
                status: $('#paymentStatus').is(':checked') ? 1 : 0,
                _token: '{{ csrf_token() }}'
            };

            if (!paymentData.student_name || !paymentData.fee_type || !paymentData.amount || !paymentData.payment_date || !paymentData.method) {
